...
app.post('/api/status', (req, res) => {
  const data = z.object({ applicationId: z.string().trim().min(1) }).safeParse(req.body)
  if (!data.success) return res.status(400).json({ error: 'Enter a valid application ID.' })
  const row = db.prepare('SELECT * FROM applications WHERE application_id = ?').get(data.data.applicationId)
  if (!row) return res.status(404).json({ error: 'No application matches that ID.' })
...
app.get('/api/admin/applications', requireAdmin, (req, res) => {
  const rows = db.prepare('SELECT application_id AS applicationId, player_name AS playerName, minecraft_username AS minecraftUsername, submitted_at AS submissionDate, status FROM applications ORDER BY submitted_at DESC').all()
  res.json(rows)
})
